...
class DDPG():
    ...
    def reset_episode(self):
        ...
        state = np.concatenate([np.array(state[:9]),
                                [state[2]],
                                keras.utils.to_categorical(state[-1], num_classes=9)]) #
    ...
    def generate_episode(self):
        ...
        state = self.reset_episode() # state: [X, Y, Z, alpha, beta, theta, dX_o, dY_o, dA_o, dZ_o, one-hot]
        goal = np.copy(state)
        goal[0] += goal[6]
        goal[1] += goal[7]
        goal[2] = 0
        ...
        while not done:
            ...
            next_state = np.concatenate([np.array(next_state[:9]), [next_state[2]],
                keras.utils.to_categorical(next_state[-1], num_classes=9)])
            state = next_state
            states.append(state) # len = len(episode) + 1
            rewards.append(reward) # len = len(episode)
            actions.append(action) # len = len(episode)
            dones.append(done) # len = len(episode)

        return states, rewards, actions, dones, goal
    ...
    def train(self):
        with open(self.log_path, "w") as f:
            ...
            while episode <= self.training_episodes:
                ...
                # generate one single episode
                states, rewards, actions, dones, goal = self.generate_episode()
                reward_sum = np.sum(rewards)
                episode_rewards_p100 += reward_sum
                ...
                for t in range(len(rewards)):
                    ...
                    # standard experience
                    self.memory.add(np.concatenate((state, goal)),
                                    action,
                                    reward,
                    ...
                    goal_ = states[t-1]
                    reward_ = self.reward_func(state, goal)
                    done_ = True if reward_ > 0 else False
                    self.memory.add(np.concatenate((state, goal_)),
                                    action,
                                    reward_,
                                    np.concatenate((next_state, goal_)),
                                    done_)
    ...
    def sample_her_goal(self, t, states):
        ...
        if (ep_length-t)<=4:
            ...
            return_states = []
            for i in range(4):
                future = np.random.randint(t,ep_length)

                goal = states[future]
                return_states.append(goal)
